...
class MazePath {
  ...
  markGrid(sourceNode, direction) {
    ...
    if (direction === "down") {
      ...
      if (destNode === null) destNode = new _maze_node__WEBPACK_IMPORTED_MODULE_0__["default"](row + 2, col, this.ctx);
      ...
      if (destNode === null) destNode = new _maze_node__WEBPACK_IMPORTED_MODULE_0__["default"](row, col + 2, this.ctx);
  ...
  generateMaze(tree, ctx) {
    ...
    setTimeout(() => {
      this.depthFirst(this.startNode, this.endNode);
    }, (tree.length + 10) * 10);
  }

  depthFirst(currNode, endNode, visited = []) {
    console.log(this, currNode, endNode, visited);
    if (currNode === endNode) return visited;
    if (visited.includes(currNode)) return null;
