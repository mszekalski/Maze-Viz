...
/* harmony import */ var _solvers_js__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_solvers_js__WEBPACK_IMPORTED_MODULE_2__);
...
class MazePath {
  ...
  markGrid(sourceNode, direction) {
    ...
    if (direction === "down") {
      this.draw(this.ctx, row + 1, col, "white");
      ...
      destNode.drawNode(this.ctx, "white");
      ...
      this.draw(this.ctx, row, col + 1, "white");
  ...
  generateMaze(tree, ctx) {
    ...
    setTimeout(() => {
      this.draw(this.ctx, startIndex.row, startIndex.col, "green");
      this.draw(this.ctx, endIndex.row, endIndex.col, "red");
  ...
  depthFirstSolution() {
    ...
    setTimeout(() => {
      ...
      let intervalId = setInterval(() => {
        ...
        if (j !== 0) {
          node.drawNode(ctx, "blue");
          this.draw(ctx, edgeRow, edgeCol, "blue");
        } else if (j === 0) {
          this.draw(ctx, edgeRow, edgeCol, "blue");
        }
        j++;
