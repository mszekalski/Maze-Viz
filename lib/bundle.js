...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const board = new _board__WEBPACK_IMPORTED_MODULE_0__["default"](ctx, 10);
});
...
class MazePath {
  ...
  depthFirstGenerate() {
    ...
    for (let i = 0; i < allVisited.length; i++) {
      let node = allVisited[i];
      setTimeout(() => {
        let nextNode = allVisited[i + 1];
        if (
          nextNode === null &&
          (node !== this.startNode && node !== this.endNode)
        ) {
