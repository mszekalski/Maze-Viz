...
class Board {
  ...
  findMinEdge(array) {
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      ]; // extract this out to helper because you do this same pattern in primsGenerate -AZ
      if (
  ...
  primsGenerate() {
    ...
    while (minimumTree.length < this.size * this.size - 1) {
      ...
      ]; // dry this line up with findMinEdge
...
__webpack_require__.r(__webpack_exports__);
class Edge {
  constructor(source, cost, direction) {
    (this.source = source), (this.cost = cost), (this.direction = direction);
  }
}
