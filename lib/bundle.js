...
class MazePath {
  ...
  isStart(node) {
    return (
      node.coords.row === this.startNode.coords.row &&
      node.coords.col === this.startNode.coords.col
    );
  }

  isEnd(node) {
    return (
      node.coords.row === this.endNode.coords.row &&
      node.coords.col === this.endNode.coords.col
    );
  }
  ...
  depthFirstGenerate() {
    ...
    let allVisited = this.allVisited;

    allVisited.forEach((node, index) => {
      setTimeout(() => {
        node.drawVisited(ctx);
      }, index * 10);
    });
    ...
    }, allVisited.length * 10);
  }

  depthFirst(currNode, endNode, visited = []) {
    this.allVisited.push(currNode);
    if (currNode === endNode) {
      return visited;
    }
