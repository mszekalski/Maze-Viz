...
class MazePath {
  ...
  isMatching(node, index) {
    if (node.row === index.row && node.col === index.col) {
      return true;
    }
  }
  ...
  generateMaze(tree, ctx) {
    ...
    tree.forEach((edge, index) => {
      ...
      if (sourceNode === null) {
        ...
        if (this.isMatching(sourceNode, startIndex)) sourceNode.start = true;
        if (this.isMatching(sourceNode, endIndex)) sourceNode.end = true;
      }
      setTimeout(() => {
