...
class Board {
  constructor(ctx, size) {
    ...
    this.mazePath = new _maze_path__WEBPACK_IMPORTED_MODULE_2__["default"](this.tree, this.size, this.ctx);
  }

  resetBoard() {
    this.mazePath.resetMazePath();
...
class MazePath {
  ...
  generateMaze(tree, ctx) {
    ...
    let setTimeout2 = setTimeout(() => {
      ...
      this.currentSolver = new _solvers_js__WEBPACK_IMPORTED_MODULE_2__["default"](
        this.ctx,
        this.startNode,
        this.endNode,
        this.grid
  ...
  depthFirstSolution() {
    ...
    let timeOut = setTimeout(() => {
      ...
      let intervalId = setInterval(() => {
        ...
        if (j !== 0) {
          this.render(ctx, edgeRow, edgeCol, "#F47D30");
          this.render(ctx, node.coords.row, node.coords.col, "#F47D30");
        } else if (j === 0) {
          this.render(ctx, edgeRow, edgeCol, "#F47D30");
        }
        j++;
    ...
    for (let dir in currNode.children) {
      if (currNode.children[dir] !== null) {
        ...
        let timeOut = setTimeout(() => {
          ...
          this.render(ctx, edgeRow, edgeCol, "#2dabf9");
    ...
    while (queue.length) {
      ...
      let timeOutTwo = setTimeout(() => {
        if (currNode !== startNode && currNode !== endNode) {
          ...
          let edgeCol = (currNode.coords.col + prevNode.coords.col) / 2;
          this.render(ctx, edgeRow, edgeCol, "#2dabf9");
          this.render(ctx, currNode.coords.row, currNode.coords.col, "#2dabf9");
        }
      }, this.steps * 10);
  ...
  breadthFirstSolution() {
    ...
    let timeOut = setTimeout(() => {
      ...
      let intervalId = setInterval(() => {
        ...
        if (j !== 0) {
          this.render(ctx, edgeRow, edgeCol, "#F47D30");
          this.render(ctx, node[0], node[1], "#F47D30");
        } else if (j === 0) {
          this.render(ctx, edgeRow, edgeCol, "#F47D30");
        }
        j++;
